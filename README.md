...
- `stub` express instance setup function/module that is called with an instance
    of `express` app *after* setting up the static root directory handler.
...
- `preStub` express instance setup function/module that is called with an instance
    of `express` app *before* setting up the static root directory handler.

   Form: same as for the `stub` option.
