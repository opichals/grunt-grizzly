...
Grizzly.prototype._createProxy = function() {
    ...
    // Publish some stuff for use in `preStub` and `stub`
    // the express module itself
    ...
    if (this._options.preStub) this._options.preStub(app);
...
Grizzly.prototype._resolveStub = function(stub) {
...
Grizzly.prototype._validateOptions = function(options) {
    ...
    if (options.preStub) {
        options.preStub = this._resolveStub(options.preStub);
    }
    if (options.stub) {
        options.stub = this._resolveStub(options.stub);
    }
